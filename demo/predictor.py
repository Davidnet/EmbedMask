    ...
    def compute_prediction(self, original_image):
        ...
        if prediction.has_field("mask"):
            ...
            if masks.dim() == 3:
                ...
                h = (masks.shape[1] * stride_mask.float() * height / input_h).ceil().long()
                w = (masks.shape[2] * stride_mask.float() * width / input_w).ceil().long()
                mask_th = prediction.get_field('mask_th').cuda()
                masks = masks.cuda()
                masks = torch.nn.functional.interpolate(input=masks.unsqueeze(1).float(), size=(h, w),
                                                        mode="bilinear", align_corners=False).gt(mask_th)
